import { checkUserService, loginUserService, registerUserService, verifyUserSevice } from "../services/user.sesrvice.js"
...
const registerUser = expressAsyncHandler(async (req, res, next) => {
    ...
    if (!name || !email || !password) return next(new AppError('All Fields are mandatory', 400))
...
const loginUser = expressAsyncHandler(async (req, res, next) => {
    ...
    if (!email || !password) return next(new AppError('All Fields are mandatory', 400))
